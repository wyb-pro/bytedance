...
function renderComponent(vdom, parent) {
  ...
  if (Component.isPrototypeOf(vdom.type)) {
    ...
    instance.dom = render(componentVdom, parent); //  记录dom是用什么组件渲染出来的

    instance.dom.__instance = instance; //  用于对元素的重用
...
function patch(dom, vdom, parent = dom.parentNode) {
  ...
  if (isComponentVdom(vdom)) {
    ...
    const active = document.activeElement; //  拍平数组，循环渲染vdom的children
    ...
    for (const attr of dom.attributes) dom.removeAttribute(attr.name); //  设置新的props
